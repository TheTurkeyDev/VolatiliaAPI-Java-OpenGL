...
public class ShadowMapMasterRenderer
{
	...
	/**
	 * Creates instances of the important objects needed for rendering the scene to the shadow map. This includes the {@link ShadowBox} which calculates the position and size of the "view cuboid", the simple renderer and shader program that are used to render objects to the shadow map, and the {@link ShadowFrameBuffer} to which the scene is rendered. The size of the shadow map is determined here.
	 * 
	 * @param camera
	...
	/**
	 * Carries out the shadow render pass. This renders the entities to the shadow map. First the shadow box is updated to calculate the size and position of the "view cuboid". The light direction is assumed to be "-lightPosition" which will be fairly accurate assuming that the light is very far from the scene. It then prepares to render, renders the entities to the shadow map, and finishes rendering.
	 * 
	 * @param entities
	 *            - the lists of entities to be rendered. Each list is associated with the {@link TexturedModel} that all of the entities in that list use.
	 * @param sun
	...
	/**
	 * This biased projection-view matrix is used to convert fragments into "shadow map space" when rendering the main render pass. It converts a world space position into a 2D coordinate on the shadow map. This is needed for the second part of shadow mapping.
	 * 
	...
	/**
	 * @return The ID of the shadow map texture. The ID will always stay the same, even when the contents of the shadow map texture change each frame.
	 */
	...
	/**
	 * Finish the shadow render pass. Stops the shader and unbinds the shadow FBO, so everything rendered after this point is rendered to the screen, rather than to the shadow FBO.
	 */
	private void finish()
	{
		shader.stop();
	...
	/**
	 * Creates the orthographic projection matrix. This projection matrix basically sets the width, length and height of the "view cuboid", based on the values that were calculated in the {@link ShadowBox} class.
	 * 
	 * @param width
	 *            - shadow box width.
	 * @param height
	...
	/**
	 * Create the offset for part of the conversion to shadow map space. This conversion is necessary to convert from one coordinate system to the coordinate system that we can use to sample to shadow map.
	 * 
